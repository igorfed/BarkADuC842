...
//not watch dog reset
    if(!bWatchDog)
		...
		//установим таймаут до сброса, длина цепи собаки
		/*
	    EADRH = 0x00;
	    EADRL = 0x08;						         
		ECON = 1;
		...
		//*/
		XBYTE[A_WD_CTRL]     = 0x03; //ЗАГРУЗКА  собаку посадили на цепь
		...
		bREADY = SelfExamine();
	  	XBYTE[A_WD_CTRL]  = 0x01;//РАЗРЕШЕНИЕ СЧЁТА
  ...
  else//собака укусила
  {
	...
	XBYTE[A_WD_CTRL]  = 0x03; //погладим собаку и посадим на цепь ЗАГРУЗКА 
	XBYTE[A_WD_STATE] = 0x00; //сбросим флаг,  
	XBYTE[A_WD_CTRL]  = 0x01; //РАЗРЕШЕНИЕ СЧЁТА 
    //читаем счётчик срабатываний по W.D.
    EADRH = 0x00;
    EADRL = 0x07;						         
	ECON = 1;
    ECON = 4;
